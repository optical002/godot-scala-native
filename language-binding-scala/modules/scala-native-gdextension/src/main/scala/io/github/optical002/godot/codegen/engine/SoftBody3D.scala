...
abstract class SoftBody3D extends MeshInstance3D {
  ...
  final def getPointTransform(point_index: Long): io.github.optical002.godot.builtin.Vector3 =
    Ptrcall.call1[Long, io.github.optical002.godot.builtin.Vector3](MethodBind.get("SoftBody3D", "get_point_transform", 871989493L), hostObject.objectPtr, point_index)

  /** SoftBody3D.apply_impulse */
  final def applyImpulse(point_index: Long, impulse: io.github.optical002.godot.builtin.Vector3): Unit =
  ...
  final def applyForce(point_index: Long, force: io.github.optical002.godot.builtin.Vector3): Unit =
