...
abstract class AudioStreamPlaybackPolyphonic extends AudioStreamPlayback {
  ...
  final def playStream(stream: AudioStream, from_offset: Double, volume_db: Double, pitch_scale: Double, playback_type: Long, bus: io.github.optical002.godot.builtin.StringName): Long =
    Ptrcall.call6[GodotObject, Double, Double, Double, Long, io.github.optical002.godot.builtin.StringName, Long](MethodBind.get("AudioStreamPlaybackPolyphonic", "play_stream", 1846744803L), hostObject.objectPtr, stream.hostObject, from_offset, volume_db, pitch_scale, playback_type, bus)
