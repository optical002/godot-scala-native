...
abstract class Animation extends Resource {
  ...
  final def positionTrackInsertKey(track_idx: Long, time: Double, position: gdext.builtin.Vector3): Long =
    Ptrcall.call3[Long, Double, gdext.builtin.Vector3, Long](MethodBind.get("Animation", "position_track_insert_key", 2540608232L), hostObject.objectPtr, track_idx, time, position)

  /** Animation.rotation_track_insert_key */
  final def rotationTrackInsertKey(track_idx: Long, time: Double, rotation: gdext.builtin.Quaternion): Long =
    Ptrcall.call3[Long, Double, gdext.builtin.Quaternion, Long](MethodBind.get("Animation", "rotation_track_insert_key", 4165004800L), hostObject.objectPtr, track_idx, time, rotation)

  /** Animation.scale_track_insert_key */
  final def scaleTrackInsertKey(track_idx: Long, time: Double, scale: gdext.builtin.Vector3): Long =
    Ptrcall.call3[Long, Double, gdext.builtin.Vector3, Long](MethodBind.get("Animation", "scale_track_insert_key", 2540608232L), hostObject.objectPtr, track_idx, time, scale)
  ...
  final def positionTrackInterpolate(track_idx: Long, time_sec: Double, backward: Boolean): gdext.builtin.Vector3 =
    Ptrcall.call3[Long, Double, Boolean, gdext.builtin.Vector3](MethodBind.get("Animation", "position_track_interpolate", 3530011197L), hostObject.objectPtr, track_idx, time_sec, backward)

  /** Animation.rotation_track_interpolate */
  final def rotationTrackInterpolate(track_idx: Long, time_sec: Double, backward: Boolean): gdext.builtin.Quaternion =
    Ptrcall.call3[Long, Double, Boolean, gdext.builtin.Quaternion](MethodBind.get("Animation", "rotation_track_interpolate", 2915876792L), hostObject.objectPtr, track_idx, time_sec, backward)

  /** Animation.scale_track_interpolate */
  final def scaleTrackInterpolate(track_idx: Long, time_sec: Double, backward: Boolean): gdext.builtin.Vector3 =
    Ptrcall.call3[Long, Double, Boolean, gdext.builtin.Vector3](MethodBind.get("Animation", "scale_track_interpolate", 3530011197L), hostObject.objectPtr, track_idx, time_sec, backward)
  ...
  final def methodTrackGetName(track_idx: Long, key_idx: Long): gdext.builtin.StringName =
    Ptrcall.call2[Long, Long, gdext.builtin.StringName](MethodBind.get("Animation", "method_track_get_name", 351665558L), hostObject.objectPtr, track_idx, key_idx)

  /** Animation.bezier_track_insert_key */
  final def bezierTrackInsertKey(track_idx: Long, time: Double, value: Double, in_handle: gdext.builtin.Vector2, out_handle: gdext.builtin.Vector2): Long =
    Ptrcall.call5[Long, Double, Double, gdext.builtin.Vector2, gdext.builtin.Vector2, Long](MethodBind.get("Animation", "bezier_track_insert_key", 3656773645L), hostObject.objectPtr, track_idx, time, value, in_handle, out_handle)
  ...
  final def bezierTrackSetKeyInHandle(track_idx: Long, key_idx: Long, in_handle: gdext.builtin.Vector2, balanced_value_time_ratio: Double): Unit =
  ...
  final def bezierTrackSetKeyOutHandle(track_idx: Long, key_idx: Long, out_handle: gdext.builtin.Vector2, balanced_value_time_ratio: Double): Unit =
  ...
  final def bezierTrackGetKeyInHandle(track_idx: Long, key_idx: Long): gdext.builtin.Vector2 =
    Ptrcall.call2[Long, Long, gdext.builtin.Vector2](MethodBind.get("Animation", "bezier_track_get_key_in_handle", 3016396712L), hostObject.objectPtr, track_idx, key_idx)

  /** Animation.bezier_track_get_key_out_handle */
  final def bezierTrackGetKeyOutHandle(track_idx: Long, key_idx: Long): gdext.builtin.Vector2 =
    Ptrcall.call2[Long, Long, gdext.builtin.Vector2](MethodBind.get("Animation", "bezier_track_get_key_out_handle", 3016396712L), hostObject.objectPtr, track_idx, key_idx)
  ...
  final def animationTrackInsertKey(track_idx: Long, time: Double, animation: gdext.builtin.StringName): Long =
    Ptrcall.call3[Long, Double, gdext.builtin.StringName, Long](MethodBind.get("Animation", "animation_track_insert_key", 158676774L), hostObject.objectPtr, track_idx, time, animation)

  /** Animation.animation_track_set_key_animation */
  final def animationTrackSetKeyAnimation(track_idx: Long, key_idx: Long, animation: gdext.builtin.StringName): Unit =
  ...
  final def animationTrackGetKeyAnimation(track_idx: Long, key_idx: Long): gdext.builtin.StringName =
    Ptrcall.call2[Long, Long, gdext.builtin.StringName](MethodBind.get("Animation", "animation_track_get_key_animation", 351665558L), hostObject.objectPtr, track_idx, key_idx)

  /** Animation.add_marker */
  final def addMarker(name: gdext.builtin.StringName, time: Double): Unit =
    Ptrcall.callVoid2(MethodBind.get("Animation", "add_marker", 4135858297L), hostObject.objectPtr, name, time)

  /** Animation.remove_marker */
  final def removeMarker(name: gdext.builtin.StringName): Unit =
    Ptrcall.callVoid1(MethodBind.get("Animation", "remove_marker", 3304788590L), hostObject.objectPtr, name)

  /** Animation.has_marker */
  final def hasMarker(name: gdext.builtin.StringName): Boolean =
    Ptrcall.call1[gdext.builtin.StringName, Boolean](MethodBind.get("Animation", "has_marker", 2619796661L), hostObject.objectPtr, name)

  /** Animation.get_marker_at_time */
  final def getMarkerAtTime(time: Double): gdext.builtin.StringName =
    Ptrcall.call1[Double, gdext.builtin.StringName](MethodBind.get("Animation", "get_marker_at_time", 4079494655L), hostObject.objectPtr, time)

  /** Animation.get_next_marker */
  final def getNextMarker(time: Double): gdext.builtin.StringName =
    Ptrcall.call1[Double, gdext.builtin.StringName](MethodBind.get("Animation", "get_next_marker", 4079494655L), hostObject.objectPtr, time)

  /** Animation.get_prev_marker */
  final def getPrevMarker(time: Double): gdext.builtin.StringName =
    Ptrcall.call1[Double, gdext.builtin.StringName](MethodBind.get("Animation", "get_prev_marker", 4079494655L), hostObject.objectPtr, time)

  /** Animation.get_marker_time */
  final def getMarkerTime(name: gdext.builtin.StringName): Double =
    Ptrcall.call1[gdext.builtin.StringName, Double](MethodBind.get("Animation", "get_marker_time", 2349060816L), hostObject.objectPtr, name)

  /** Animation.get_marker_color */
  final def getMarkerColor(name: gdext.builtin.StringName): gdext.builtin.Color =
    Ptrcall.call1[gdext.builtin.StringName, gdext.builtin.Color](MethodBind.get("Animation", "get_marker_color", 3742943038L), hostObject.objectPtr, name)

  /** Animation.set_marker_color */
  final def setMarkerColor(name: gdext.builtin.StringName, color: gdext.builtin.Color): Unit =
