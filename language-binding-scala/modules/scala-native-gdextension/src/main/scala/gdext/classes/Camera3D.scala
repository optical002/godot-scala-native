...
abstract class Camera3D extends Node3D {
  ...
  final def projectRayNormal(screen_point: gdext.builtin.Vector2): gdext.builtin.Vector3 =
    Ptrcall.call1[gdext.builtin.Vector2, gdext.builtin.Vector3](MethodBind.get("Camera3D", "project_ray_normal", 1718073306L), hostObject.objectPtr, screen_point)

  /** Camera3D.project_local_ray_normal */
  final def projectLocalRayNormal(screen_point: gdext.builtin.Vector2): gdext.builtin.Vector3 =
    Ptrcall.call1[gdext.builtin.Vector2, gdext.builtin.Vector3](MethodBind.get("Camera3D", "project_local_ray_normal", 1718073306L), hostObject.objectPtr, screen_point)

  /** Camera3D.project_ray_origin */
  final def projectRayOrigin(screen_point: gdext.builtin.Vector2): gdext.builtin.Vector3 =
    Ptrcall.call1[gdext.builtin.Vector2, gdext.builtin.Vector3](MethodBind.get("Camera3D", "project_ray_origin", 1718073306L), hostObject.objectPtr, screen_point)

  /** Camera3D.unproject_position */
  final def unprojectPosition(world_point: gdext.builtin.Vector3): gdext.builtin.Vector2 =
    Ptrcall.call1[gdext.builtin.Vector3, gdext.builtin.Vector2](MethodBind.get("Camera3D", "unproject_position", 3758901831L), hostObject.objectPtr, world_point)

  /** Camera3D.is_position_behind */
  final def isPositionBehind(world_point: gdext.builtin.Vector3): Boolean =
    Ptrcall.call1[gdext.builtin.Vector3, Boolean](MethodBind.get("Camera3D", "is_position_behind", 3108956480L), hostObject.objectPtr, world_point)

  /** Camera3D.project_position */
  final def projectPosition(screen_point: gdext.builtin.Vector2, z_depth: Double): gdext.builtin.Vector3 =
    Ptrcall.call2[gdext.builtin.Vector2, Double, gdext.builtin.Vector3](MethodBind.get("Camera3D", "project_position", 2171975744L), hostObject.objectPtr, screen_point, z_depth)
  ...
  final def setFrustum(size: Double, offset: gdext.builtin.Vector2, z_near: Double, z_far: Double): Unit =
  ...
  final def getCameraTransform(): gdext.builtin.Transform3D =
    Ptrcall.call0[gdext.builtin.Transform3D](MethodBind.get("Camera3D", "get_camera_transform", 3229777777L), hostObject.objectPtr)

  /** Camera3D.get_camera_projection */
  final def getCameraProjection(): gdext.builtin.Projection =
    Ptrcall.call0[gdext.builtin.Projection](MethodBind.get("Camera3D", "get_camera_projection", 2910717950L), hostObject.objectPtr)
  ...
  final def getFrustumOffset(): gdext.builtin.Vector2 =
    Ptrcall.call0[gdext.builtin.Vector2](MethodBind.get("Camera3D", "get_frustum_offset", 3341600327L), hostObject.objectPtr)
  ...
  final def setFrustumOffset(offset: gdext.builtin.Vector2): Unit =
  ...
  final def isPositionInFrustum(world_point: gdext.builtin.Vector3): Boolean =
    Ptrcall.call1[gdext.builtin.Vector3, Boolean](MethodBind.get("Camera3D", "is_position_in_frustum", 3108956480L), hostObject.objectPtr, world_point)
