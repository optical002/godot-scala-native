...
abstract class AtlasTexture extends Texture2D {
  ...
  final def setRegion(region: gdext.builtin.Rect2): Unit =
    Ptrcall.callVoid1(MethodBind.get("AtlasTexture", "set_region", 2046264180L), hostObject.objectPtr, region)

  /** AtlasTexture.get_region */
  final def getRegion(): gdext.builtin.Rect2 =
    Ptrcall.call0[gdext.builtin.Rect2](MethodBind.get("AtlasTexture", "get_region", 1639390495L), hostObject.objectPtr)

  /** AtlasTexture.set_margin */
  final def setMargin(margin: gdext.builtin.Rect2): Unit =
    Ptrcall.callVoid1(MethodBind.get("AtlasTexture", "set_margin", 2046264180L), hostObject.objectPtr, margin)

  /** AtlasTexture.get_margin */
  final def getMargin(): gdext.builtin.Rect2 =
    Ptrcall.call0[gdext.builtin.Rect2](MethodBind.get("AtlasTexture", "get_margin", 1639390495L), hostObject.objectPtr)
