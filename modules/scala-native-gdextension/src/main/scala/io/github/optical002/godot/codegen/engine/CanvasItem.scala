...
/** Generated wrapper for Godot's `CanvasItem`, inherits `Node`. */
opaque type CanvasItem = GodotObject

object CanvasItem {
  /** Wrap a raw object handle as a `CanvasItem` (no checked cast). */
  def fromObject(o: GodotObject): CanvasItem = o
  ...
  given GodotClass[CanvasItem] with {
    ...
    def wrap(o: GodotObject): CanvasItem = o
    def unwrap(t: CanvasItem): GodotObject = t
  }

  extension (self: CanvasItem) {
