...
/** Generated wrapper for Godot's `Node2D`, inherits `CanvasItem`. */
opaque type Node2D = GodotObject

object Node2D {
  /** Wrap a raw object handle as a `Node2D` (no checked cast). */
  def fromObject(o: GodotObject): Node2D = o

  /** Class metadata for Gd[Node2D] lifetime management and casting. */
  given GodotClass[Node2D] with {
    def className = "Node2D"
    def isRefCounted = false
    def wrap(o: GodotObject): Node2D = o
    def unwrap(t: Node2D): GodotObject = t
  }

  extension (self: Node2D) {
