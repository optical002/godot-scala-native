...
object MethodRegistration {
  ...
  private def fillPropertyInfo(
    ...
    !(!pi).at_class_name = StringNames.cached("").ptr
    !(!pi).at_hint_string = emptyGString()
  }

  /** A heap GString handle holding "" (PropertyInfo wants a String hint). */
  private[register] def emptyGString(): GDExtensionStringPtr = {
...
/** PROPERTY_USAGE flags (subset). */
object PropertyUsage {
  // PROPERTY_USAGE_DEFAULT = STORAGE | EDITOR (6) in Godot.
  val Default: CUnsignedInt = 6.toUInt
}
